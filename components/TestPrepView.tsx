...
const TestPrepView: React.FC<TestPrepViewProps> = ({ subject, grade, initialSharedData, onClearInitialData }) => {
  ...
  const savePlan = (plan: TestPrepPlan | null) => {
    setActivePlan(plan);
    if (plan) {
      localStorage.setItem(`test_prep_${subject}`, JSON.stringify(plan));
    } else {
      localStorage.removeItem(`test_prep_${subject}`);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[3rem] shadow-sm min-h-[400px] text-center border border-gray-100">
        ...
        <h3 className="text-3xl font-black text-gray-900 mb-2">בונה לך תוכנית אישית...</h3>
        <p className="text-gray-700 font-bold mb-4 text-xl">יקח עד דקה</p>
        <p className="text-gray-400 text-sm max-w-xs mx-auto">הבינה המלאכותית מנתחת את החומר ובונה חווית למידה מותאמת אישית.</p>
      </div>
    );
  }
          ...
          <div className="bg-gray-900 text-white p-10 md:p-14 relative overflow-hidden">
            ...
            <div className="relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                    <div><span className="bg-blue-500 text-white px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-widest mb-4 inline-block shadow-lg">יום {day.dayNumber} מתוך {activePlan.totalDays}</span><h2 className="text-4xl md:text-5xl font-black">{day.title}</h2></div>
                    <div className="flex items-center gap-3 bg-white/10 px-5 py-3 rounded-2xl backdrop-blur-md border border-white/10"><Clock className="text-blue-400" size={20} /><span className="text-sm font-bold">כ-40 דקות למידה אינטראקטיבית</span></div>
                </div>
                <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
                    <button onClick={() => setActiveSubTab('CONTENT')} className={`px-8 py-3 rounded-2xl text-sm font-black transition-all flex items-center gap-3 shrink-0 ${activeSubTab === 'CONTENT' ? 'bg-white text-gray-900 shadow-xl' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}><BookOpen size={20} /><span>סיכום</span></button>
                    <button onClick={() => setActiveSubTab('INTERACTIVE')} className={`px-8 py-3 rounded-2xl text-sm font-black transition-all flex items-center gap-3 shrink-0 ${activeSubTab === 'INTERACTIVE' ? 'bg-white text-gray-900 shadow-xl' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}><Layers size={20} /><span>אינטראקטיבי</span></button>
                    <button onClick={() => setActiveSubTab('QUIZ')} className={`px-8 py-3 rounded-2xl text-sm font-black transition-all flex items-center gap-3 shrink-0 ${activeSubTab === 'QUIZ' ? 'bg-white text-gray-900 shadow-xl' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}><FileText size={20} /><span>תרגול</span></button>
                </div>
            </div>
          </div>
          <div className="p-8 md:p-14 bg-gray-50/30">
            {activeSubTab === 'CONTENT' && (
              <section className="animate-fade-in space-y-10">
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-4"><div className="flex items-center gap-4"><div className="bg-blue-100 p-3 rounded-2xl text-blue-600 shadow-sm"><BookOpen size={28} /></div><div><h3 className="text-2xl font-black text-gray-900">סיכום החומר</h3><p className="text-sm text-gray-500">קרא בעיון את הדגשים החשובים ליום זה</p></div></div><a href={youtubeSearchUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 bg-red-50 text-red-600 px-6 py-3 rounded-2xl font-black text-sm hover:bg-red-100 transition-all shadow-sm border border-red-100 group"><Youtube size={22} className="group-hover:scale-110 transition-transform" />צפה בשיעור וידאו רלוונטי</a></div>
            ...
            {activeSubTab === 'INTERACTIVE' && (
              <section className="animate-fade-in space-y-16">
                <div><div className="flex items-center gap-4 mb-10"><div className="bg-orange-100 p-3 rounded-2xl text-orange-600 shadow-sm"><Layers size={28} /></div><div><h3 className="text-2xl font-black text-gray-900">כרטיסיות חזרה מהירה</h3><p className="text-sm text-gray-500">בחן את עצמך על המושגים המרכזיים</p></div></div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{day.flashcards.map((card, idx) => <FlashcardComp key={idx} index={idx} card={card} />)}</div></div>
                <div className="pt-16 border-t border-gray-100"><div className="flex items-center gap-4 mb-10"><div className="bg-emerald-100 p-3 rounded-2xl text-emerald-600 shadow-sm"><MapIcon size={28} /></div><div><h3 className="text-2xl font-black text-gray-900">מפת מושגים אינטראקטיבית</h3><p className="text-sm text-gray-500">הבנת הקשרים והתהליכים המרכזיים בנושא</p></div></div><div className="bg-white rounded-[3rem] p-10 md:p-16 border border-gray-100 shadow-lg relative overflow-hidden"><div className="relative z-10"><ConceptMapVisual links={day.conceptMap} /></div></div></div>
              </section>
            )}
            {activeSubTab === 'QUIZ' && (
              <section className="animate-fade-in space-y-12 max-w-4xl mx-auto"><div className="flex items-center gap-4 mb-8"><div className="bg-purple-100 p-3 rounded-2xl text-purple-600 shadow-sm"><FileText size={28} /></div><div><h3 className="text-2xl font-black text-gray-900">תרגול שאלות בחינה</h3><p className="text-sm text-gray-500">שאלות ברמת בחינה המבוססות על החומר שלמדת</p></div></div><div className="space-y-8">{day.quiz.map((q, qIdx) => (
                    <div key={qIdx} className="bg-white p-8 md:p-10 rounded-[2.5rem] border-2 border-gray-100 shadow-sm hover:border-purple-200 transition-all"><div className="flex items-start gap-4 mb-6"><div className="bg-purple-50 text-purple-600 w-10 h-10 rounded-2xl flex items-center justify-center font-black text-sm shrink-0 shadow-sm">{qIdx + 1}</div><h4 className="font-bold text-gray-900 text-xl leading-tight pt-1"><LatexRenderer text={q.text} /></h4></div><div className="grid gap-4">{q.options.map((opt, oIdx) => {
                        ...
                        })}</div>{quizFinished && (<div className="mt-8 p-6 bg-blue-50/50 rounded-2xl border border-blue-100 flex gap-4 animate-fade-in"><Lightbulb className="text-blue-500 shrink-0" size={24} /><div className="text-sm text-blue-900 leading-relaxed"><strong className="block mb-1 text-base">הסבר הפתרון:</strong><LatexRenderer text={q.explanation} /></div></div>)}</div>
                  ))}</div>
                {!quizFinished ? ( <button disabled={Object.keys(quizAnswers).length < day.quiz.length} onClick={() => setQuizFinished(true)} className="w-full mt-10 bg-gray-900 text-white py-6 rounded-3xl font-black text-xl shadow-2xl hover:shadow-gray-400 hover:-translate-y-1 transition-all disabled:opacity-30 disabled:cursor-not-allowed">בדוק תשובות וקבל ציון</button> ) : ( <button onClick={() => handleCompleteDay(day.dayNumber)} className="w-full mt-10 bg-green-600 text-white py-6 rounded-3xl font-black text-xl shadow-2xl hover:bg-green-700 hover:-translate-y-1 transition-all flex items-center justify-center gap-3"><CheckCircle size={28} /><span>סיימתי את היום בהצלחה!</span></button> )}
              </section>
            )}
          </div>
        </div>
      ...
      <div className="animate-fade-in max-w-5xl mx-auto space-y-8 pb-20">
        <div className="bg-white rounded-[3rem] p-10 md:p-14 shadow-xl border border-gray-100 flex flex-col md:flex-row justify-between gap-8 relative overflow-hidden">
          ...
          <div className="flex-1 relative z-10">
            <div className="flex items-center gap-3 text-blue-500 font-black text-xs uppercase tracking-widest mb-4"><Sparkles size={16} /><span>תוכנית למידה פעילה</span></div>
            <h2 className="text-4xl font-black text-gray-900 mb-2">{activePlan.targetTopic}</h2>
            <p className="text-gray-500 font-medium">תוכנית הכנה אישית ל{activePlan.subject} ב{activePlan.totalDays} ימים</p>
            <div className="mt-8"><div className="flex justify-between items-center mb-3"><span className="text-sm font-black text-gray-600">התקדמות</span><span className="text-sm font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-full">{progress}%</span></div><div className="h-4 bg-gray-100 rounded-full overflow-hidden shadow-inner"><div className="h-full bg-gradient-to-r from-blue-500 to-indigo-500 transition-all duration-1000 shadow-lg" style={{ width: `${progress}%` }}></div></div></div>
          </div>
          <button onClick={() => { if(confirm("האם למחוק את התוכנית הקיימת ולהתחיל מחדש?")) savePlan(null); }} className="text-red-400 hover:text-red-600 text-sm font-black flex items-center gap-2 self-start bg-red-50 px-4 py-2 rounded-xl transition-all hover:bg-red-100"><RotateCcw size={16} />בטל והתחל מחדש</button>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 animate-fade-in pb-20">
      <div className="bg-white rounded-[3rem] p-12 md:p-16 shadow-2xl border border-gray-100 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50 rounded-bl-[4rem] -z-0"></div>
        <div className="relative z-10">
          <div className="flex items-center gap-6 mb-12"><div className="bg-blue-100 p-5 rounded-[2rem] text-blue-600 shadow-inner"><Calendar size={40} /></div><div><h2 className="text-4xl font-black text-gray-900 mb-1">הכנה למבחן</h2><p className="text-gray-500 text-lg font-medium">בוא נבנה לך תוכנית למידה מנצחת</p></div></div>
          <div className="space-y-8">
            <div><label className="block text-sm font-black text-gray-700 mb-3 uppercase tracking-widest">על מה המבחן?</label><textarea value={topicInput} onChange={(e) => setTopicInput(e.target.value)} placeholder="לדוגמה: משוואות ריבועיות, המהפכה התעשייתית..." className="w-full p-6 bg-gray-50 border-2 border-gray-100 rounded-[2rem] focus:border-primary focus:bg-white focus:ring-4 focus:ring-primary/10 outline-none transition-all h-32 resize-none mb-4 font-medium text-lg" />
                <div className="flex flex-col gap-3"><div className="flex items-center gap-4"><button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-3 px-6 py-3 bg-white hover:bg-gray-50 text-gray-800 rounded-2xl transition-all font-black text-sm border-2 border-gray-100 shadow-sm group"><Paperclip size={20} className="group-hover:rotate-12 transition-transform" /><span>העלה קובץ (סיכומי מורה / דפי חזרה)</span></button><input type="file" ref={fileInputRef} onChange={handleFileSelect} accept="image/*,application/pdf" className="hidden" /></div>{attachment && (<div className="flex items-center gap-3 bg-blue-600 text-white px-5 py-3 rounded-2xl w-fit animate-fade-in shadow-lg"><FileText size={18} /><span className="text-sm font-black max-w-[200px] truncate">{attachment.name}</span><button onClick={removeAttachment} className="hover:bg-white/20 p-1.5 rounded-full transition-colors ml-1"><X size={16} /></button></div>)}</div>
            </div>
            <div className="grid md:grid-cols-2 gap-8"><div><label className="block text-sm font-black text-gray-700 mb-3 uppercase tracking-widest">כמה ימים נשארו?</label><div className="flex items-center gap-6 bg-gray-50 p-3 rounded-[1.5rem] border-2 border-gray-100"><button onClick={() => setDaysCount(Math.max(1, daysCount - 1))} className="w-12 h-12 bg-white rounded-2xl shadow-sm font-black text-2xl hover:bg-gray-100 transition-all">-</button><span className="flex-1 text-center font-black text-3xl text-gray-800">{daysCount}</span><button onClick={() => setDaysCount(Math.min(14, daysCount + 1))} className="w-12 h-12 bg-white rounded-2xl shadow-sm font-black text-2xl hover:bg-gray-100 transition-all">+</button></div></div></div>
            <button onClick={() => handleCreatePlan()} disabled={(!topicInput.trim() && !attachment) || loading} className="w-full bg-gray-900 text-white py-6 rounded-[2rem] font-black text-2xl shadow-2xl hover:bg-black hover:-translate-y-1 transition-all flex items-center justify-center gap-4 mt-6 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed"><Sparkles size={32} />צור תוכנית לימודים חכמה</button>
          </div>
        </div>
      </div>
    </div>
